...
func Test_AesEnryption(t *testing.T) {
	...
	for i, plain1 := range testData {
		...
		t.Run(name, func(t *testing.T) {
			cipher, err := DoAesECBEncrypt([]byte(plain1), key)
			assert.Nil(t, err)
			if err == nil {
				plain2, err := DoAesECBDecrypt(cipher, key)
				assert.Nil(t, err)
				if err == nil {
...
func Test_AesCBCEnryption(t *testing.T) {
	...
	for i, plain1 := range testData {
		name := fmt.Sprintf("Name %d", i)
		t.Run(name, func(t *testing.T) {
			cipher, err := DoAesCBCEncrypt([]byte(plain1), key)
			assert.Nil(t, err)
			if err == nil {
				plain2, err := DoAesCBCDecrypt(cipher, key)
				assert.Nil(t, err)
				if err == nil {
					plainText2 := string(plain2)
					assert.Equal(t, plain1, plainText2)
