...
#docker run -d --network $NETWORK $VOLUME_MOUNT_ARG -e STRICT_CONNECTION_CHECK=true -e LOG_LEVEL=trace  --name $HTTPPROXYSERVER -p 8082:8080 -e NATS_SERVER_URL=$CLOUDNATSURL $HTTPPROXYSERVER_FULL_IMG
...
docker run -d --network $NETWORK $VOLUME_MOUNT_ARG --name $NGINXTEST -p 80:80 -p 443:442 $REPO/$NGINXTEST:$IMAGE_TAG
sleep 30
echo "*** RUNNING PODS"
docker ps
