...
docker run -d --network $NETWORK --name $HTTPPROXYSERVER -p 8084:8080 -e NATS_SERVER_URL=$CLOUDNATSURL $REPO/$HTTPPROXYSERVER:$IMAGE_TAG
...
docker run -d --network $NETWORK --name $SYNCCLIENT -p 8082:8080 -e LOG_LEVEL=trace -e KEYSTORE_URL=$ONPREMKEYSTOREURL -e NATS_SERVER_URL=$ONPREMNATSURL -e CLOUD_BRIDGE_URL=$CLOUD_BRIDGE_URL $REPO/$SYNCCLIENT:$IMAGE_TAG
docker run -d --network $NETWORK --name $HTTPPROXYLET -e DEFAULT_LOCATION_ID="*" -e NATS_SERVER_URL=$ONPREMNATSURL $REPO/$HTTPPROXYLET:$IMAGE_TAG
docker run -d --network $NETWORK --name $NGINXTEST $REPO/$NGINXTEST:$IMAGE_TAG
