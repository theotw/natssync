...
TEST_BINARIES_DIR="${TEST_BINARIES_DIR:-out}"

[ "${1}" == "" ] && echo "Error: invalid first argument" && echo "Usage example: go_test_app_for_coverage.sh apps/bridge_client_test.go" && exit 1

app_test_filepath="${1}"  # 'test/apps/bridge_client_test.go'
#app_test_dir=$(dirname "${app_test_filepath}") # 'test/apps'
app_test_filename=$(basename -- "${app_test_filepath}")  # 'bridge_client_test.go'
app_test_name="${app_test_filename%.*}"  # 'mytestsuite_bridge_client_test'
app_test_binary_filepath="${TEST_BINARIES_DIR}/${app_test_filename%.*}.test"
...
#    rm -rf "${app_test_dir}/openapi" "${app_test_dir}/third_party"
#}

echo " -- Building test binary: ${app_test_binary_filepath}"
...
#cp -r openapi/ "${app_test_dir}/"
#mkdir -p "${app_test_dir}/third_party" && cp -r third_party/swaggerui/ "${app_test_dir}/third_party/"

rm -f "${app_test_binary_filepath}"
go test -c -o "${app_test_binary_filepath}" -v "${app_test_filepath}" -coverprofile="${go_coverage_filepath}" -coverpkg=./pkg/...
